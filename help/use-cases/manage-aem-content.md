...
description: Utilizza AEM MCP Server per trovare, rivedere, aggiornare e pubblicare contenuti AEM senza dover passare da un strumento all’altro.
...
Mantenere aggiornato il contenuto del sito web è una pressione operativa costante. Questa procedura dettagliata mostra come i team di contenuto possono trovare, rivedere, aggiornare e pubblicare pagine e frammenti di contenuto di AEM tramite un client AI, utilizzando AEM MCP Server per ridurre il tempo tra una decisione sui contenuti e un aggiornamento live.
...
| Strumenti agentici | [CX Enterprise Coworker](https://experienceleague.adobe.com/it/docs/cx-enterprise-coworker/content/home) o [AEM MCP Server](https://experienceleague.adobe.com/it/docs/experience-manager-learn/cloud-service/ai/mcp-servers/overview) |
...
Il modo più veloce per farlo è CX Enterprise Coworker, che non richiede alcuna configurazione del server o del client AI. [Prova CX Enterprise Coworker](https://experienceleague.adobe.com/it/docs/cx-enterprise-coworker/content/home)

Se preferisci collegare il tuo client di intelligenza artificiale direttamente ad AEM, consulta le schede di seguito.

>[!TAB Claude.ai]

Connetti il server AEM MCP come connettore personalizzato.

1. Vai a **Impostazioni > Integrazioni** in Claude.ai.
2. Seleziona **Aggiungi connettore personalizzato** e immetti l&#39;URL del server: `https://mcp.adobeaemcloud.com/adobe/mcp/aem`
...
Connetti il server AEM MCP utilizzando la modalità sviluppatore ChatGPT (è necessario un piano Pro, Plus, Business, Enterprise o Education).
...
3. Immettere l&#39;URL del server: `https://mcp.adobeaemcloud.com/adobe/mcp/aem`
...
Usare Gemini, Microsoft Copilot, Cursore, Claude Code o un altro ambiente compatibile con MCP? Connettiti al server AEM MCP utilizzando questo endpoint:
...
>[!CAUTION]
>
>Quando richiesto, confermare ogni modifica. Il server AEM MCP può creare, aggiornare ed eliminare contenuti. Rivedi la modifica proposta prima di approvarla, in particolare sulle pagine live.
...
Hai utilizzato il server MCP di AEM per trovare i contenuti, esaminare una pagina live, applicare i miglioramenti suggeriti dall’intelligenza artificiale e pubblicare il risultato, senza aprire l’interfaccia di AEM. Combinando l’individuazione, la modifica e la pubblicazione dei contenuti in un’unica sessione di intelligenza artificiale, i team di contenuto possono passare dall’identificazione di un gap alla spedizione e alla distribuzione di un aggiornamento più rapidamente e con un numero inferiore di opzioni di contesto. Lo stesso flusso di lavoro viene ridimensionato a più pagine, frammenti di contenuto e avvii coordinati di campagne.

## Più risultati da ottenere

Il server MCP di AEM gestisce molto di più delle copertine della procedura dettagliata. Espandi uno scenario qui sotto per visualizzare i prompt che puoi provare nella stessa sessione.
...
| --- | --- |
| [Documentazione del server AEM MCP](https://experienceleague.adobe.com/it/docs/experience-manager-learn/cloud-service/ai/mcp-servers/overview){target="_blank"} | Elenco strumenti e disponibilità |
